...
$(document).ready(function() {
    ...
    saveComment = saveCommentButton.on("click", function() {
        ...
        if (storedComment === "") {
            commentNotice.text(currentComment === "" ? "Try again." : "Removed Comment!");
        } else {
            commentNotice.text(currentComment === "" ? "Comment Saved!" : "Comment Changed!");
        }
    ...
    checkout = checkoutButton.on("click", function(e) {
        ...
        let totalCost = parseFloat($(".total-cost").text().replace(/[^\d.]/g, ""));
        ...
        productItem.each(function() {
            ...
            productList.append(`<li>${productName} (${quantity}) - ${totalPrice}</li>`);
        });
